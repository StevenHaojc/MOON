...
def uniformer_small_mtask(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[3, 4, 8, 3],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), aux_head=True, **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def uniformer_small_IL(num_classes=4, 
                       num_phase=1,
                       pretrained=None, 
                       pretrained_cfg=None,
                       **kwards):
...
@register_model
def uniformer_small_IL_mtask(num_classes=4, 
                       num_phase=1,
                       pretrained=None, 
                       pretrained_cfg=None,
                       **kwards):
    ...
    Concat multi-phase images with image-level
    '''
    model = uniformer_small_mtask(in_chans=num_phase, num_classes=num_classes, **kwards)
    return model

@register_model
def uniformer_base_IL(num_classes=4, 
                      num_phase=1,
                      pretrained=None, 
                      pretrained_cfg=None,
                      **kwards):
    ...
    Concat multi-phase images with image-level
    '''
    model = uniformer_base(in_chans=num_phase, num_classes=num_classes, **kwards)
    return model
