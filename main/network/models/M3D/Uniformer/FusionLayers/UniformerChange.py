...
# TODO: add aux-head
class UniFormerChange(nn.Module):
    """ Vision Transformer
    ...
    def forward_feature2(self, x):
        ...
        for blk in self.blocks2:
            x = blk(x)
        return x
    def forward_feature3(self, x):
        x = self.patch_embed3(x)
        for blk in self.blocks3:
            x = blk(x)
        return x
    def forward_feature4(self, x):
    ...
    def forward_last(self, x):
        ...
        return x
    def forward(self, x):
        x = self.forward_feature1(x)
        x = self.forward_feature2(x)
        x = self.forward_feature3(x)
        x = self.forward_feature4(x)
